...
import org.apache.ibatis.reflection.testdemo.bridge.BridgeTest;
import org.apache.ibatis.reflection.testdemo.java8.Test4ComputeIfAbsent;
...
public class Reflector {
  ...
  private void addGetMethods(Class<?> cls) {
    ...
    for (Method method : methods) {
      ...
      // 如果是开头结尾是 get（名字长度肯定要大于3）和is（名字长度肯定要大于2）
  ...
  /**
   * computeIfAbsent 的使用可以查看 {@link Test4ComputeIfAbsent}
   */
  ...
  /**
   * 初始化 getMethods + getTypes 和 setMethods + setTypes ，通过遍历 fields 属性。实际上，
   * 它是 #addGetMethods(...) 和 #addSetMethods(...) 方法的补充，因为有些 field ，不存在对应的 setting 或 getting 方法，所以直接使用对应的 field ，
   * 而不是方法
   * @param clazz
   */
